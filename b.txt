...
----------------------------------------------------------------
-- getgenv expose so re-runs can clean up
----------------------------------------------------------------
...
LIBRARY SETUP
-------------
    local Library = getgenv().Library  -- or just use the global `Library`
    local Window    = Library:Window({ Logo = "<rbxasset id>", FadeTime = 0.3 })
    local Watermark = Library:Watermark("text")
    local KeybindList = Library:KeybindList()
    local SettingsPage = Library:CreateSettingsPage(Window, Watermark, KeybindList)

PAGES
-----
    Window:Page({ Name = "Main",    Columns = 2 })     -- columned page
    Window:Page({ Name = "Secondary", SubPages = true }) -- page with sub-pages
    Page:SubPage({ Name = "Sub A", Columns = 2 })

SECTIONS
--------
    local Section = Page:SubPage or Page
    Section:Section({ Name = "Name", Side = 1 })   -- Side: column index (1, 2, ...)

ELEMENTS  (all accept Flag, Default, Callback; key is case-insensitive)
--------
    Section:Toggle({ Name=, Default=, Callback=function(bool) end })
      :Keybind({ Default=Enum.KeyCode.X, Mode="Toggle"/"Hold", Callback=fn })

    Section:Slider({ Name=, Min=, Max=, Default=, Decimals=, Suffix=, Callback=function(num) end })

    Section:Dropdown({ Name=, Items={...}, Default=, Multi=bool, Callback=function(val) end })

    Section:Button():Add("Label", function() end)   -- stack multiple buttons

    Section:Label("text")                           -- returns a label element

    Section:Textbox({ Name=, Default=, Placeholder=, Numeric=bool, Finished=bool, Callback=function(str) end })

    Section:Searchbox({ Name=, Items={...}, Default=, Multi=bool, Callback=function(val) end })

    Section:Label("text"):Colorpicker({ Default=Color3.fromRGB(...), Alpha=, Callback=function(color, alpha) end })
    Section:Label("text"):Keybind({ Default=, Mode=, Callback=fn })

UTILITY
-------
    Library:Notification(title, description, duration)
    Library:InventoryViewer()                 -- player inventory viewer
    Library.Flags                             -- table of all flag values
    Library:Unload()

    Menu toggle key:  RightControl  (rebindable via Settings -> UI Keybind)
]]
